--- 
title: "Applied Demographic Data Analysis with R - Chapter 1 Draft"
author:
...
mydat%>%
  gt()%>%
  tab_header(title= "My simple gt table", subtitle = "With a subtitle")
```
...
```{r, echo=T, results='hide', message=F, tidy=TRUE}
prb<-read.csv(file = "https://raw.githubusercontent.com/coreysparks/r_courses/master/2018_WPDS_Data_Table_FINAL.csv", stringsAsFactors = T)
```
...
desc2<- Hmisc::describe(prb, tabular=T)
head(desc2)
...
From this summary, we see that the mean is `r mean(prb$tfr, na.rm=T)`,  there is one country missing the Total fertility rate variable. The minimum is 1 and the maximum is 7.2 children per woman.
...
```{r}
var(prb$tfr, na.rm=T) #variance
sd(prb$tfr, na.rm=T) #standard deviation
...
ggplot( mapping = aes(x= continent, y =tfr))+
  geom_boxplot()+
  ggtitle(label = "Distribution of the Total Fertility Rate by Continent", subtitle = "2018 Estimates")+coord_flip()
