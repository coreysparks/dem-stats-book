...
Tabular output from our survey data analysis is possible through several different means. When using dplyr, our intermediate output of the anlaysis is always a data frame, and so any R method for printing data frames would work for simple tabular display. For instance, if we just use `knitr::kable()` on our workflow from above instead of piping into a plot we would get something like this:
...
merged%>%
  ggplot()+
  geom_bar(aes(y = emp_rate_se, x = met_name, fill = estimate),
           stat = "identity",
           position="dodge")
merged%>%
  ...
  labs(x = "MSA", 
       y = "Unemployment Rate Estimate",
       title = "Unmployment rate in Texas MSAs",
       subtitle = "Weighted vs Unweighted Estimates")+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
## How sampling affects regression - a simple first look
