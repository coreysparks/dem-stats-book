...
```{r, eval=FALSE}
glmqp <- glm(Deaths  ~offset(log(Population)) + factor(Age_Group)+State,
              data=txca,
              family=quasipoisson)
...
glmp_d <- glm.poisson.disp(glmp_s)
...
AIC(glmb_s, glmp_s, glmnb)

glmn<- glm(Deaths/Population ~ factor(Age_Group)+State,
           family=gaussian,
               data=txca)
summary(fitted(glmn)<0)
```
