...
summary(lm3)
```
Which also provides similar results to `lm1`. So, which model is better for this particular outcome? One way to examine relative model fit is to compare the Akaike Informaiton Criteria for the three models. Since the three models `lm1g`, `lm2` and `lm3` are all fit by `gls()`, we can compare them.  
