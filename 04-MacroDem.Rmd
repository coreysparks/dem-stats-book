...
Where each of the $F_k (x)$ functions imply a different form in the interval between $x\in [x_{k-1}, x_k]$, where the $k$ breaks are at a given knot in the data. Most splines are nonlinear functions, usustally cubic polynomials, and the spline model combines a series of these polynomials to model nonlinearities in the relationship between predictors and outcomes. A relatively recent invention, *Genearlized Additive Models* or *GAMs* are a way to model an outcome with both linear and non-linear terms together. The GAM model forms the linear predctor of a model can be constructed as:
...
glmb<- glm(cbind(lowbw1618, births1618-lowbw1618) ~  fampov14 + rucc + hpsa16,
          ...
          family = binomial)
summary(glmb)
pchisq(deviance(glmb), df.residual(glmb), lower=F)
pchisq(deviance(mod2), df.residual(mod2), lower=F)

plot(density(residuals(glmb, type="pearson")))
plot(density(rstandard(glmb, type="deviance")))
library(statmod)
scatter.smooth(predict(glmb, type='response'), rstandard(glmb, type='deviance'), col='gray')

scatter.smooth(sqrt(predict(glmb, type='response')), qresid(glmb), col='gray')
scatter.smooth(predict(glmb, type='response'), qresid(glmb), col='gray')
scatter.smooth(predict(glmb, type='response')~ ahrf_m$fampov14, col='gray')
plot(residuals(glmb)~ predict(glmb, type="link"))
plot(predict(glmb)~residuals(glmb, type="pearson"))
plot(predict(mod2)~residuals(mod2, type="pearson"))
```

```{r}
library(dispmod)
mod2 <- glm.binomial.disp(glmb)

mod2$dispersion
summary(mod2)


glmp<- glm(lowbw1618 ~ offset(log( births1618))+ fampov14 + rucc + hpsa16,
          data = ahrf_m, 
          family = poisson)
summary(glmp)

library(dispmod)
mod3 <- glm.poisson.disp(glmp)
...
```{r}
rg <- ref_grid(glmb, at=list( fampov14 = c(5, 6) ) ) 
emmeans(rg, specs = "fampov14", type = "response")
```
...
The Binomial is very useful for conducting standardization of rates between groups to measure the differences. 
...
To show an example of how to do age standardization, I use data from the [CDC Wonder Compressed Mortality file](https://wonder.cdc.gov/mortSQL.html). The data are for the states of Texas and California, and are the numbers of deaths and population at risk in 13 age groups. 

```{r}
txca <- readr::read_delim(url("https://raw.githubusercontent.com/coreysparks/dem-stats-book/master/data/CMF_TX_CA_age.txt?token=GHSAT0AAAAAABQZ5VMKYY4UY4W35G4LJWAAYQZAPEA"),
                          delim = "\t",
                          quote = "\"",
                          skip=1,
                          col_names = c("State",
...
To do age standardization using the regression framework, we have to control for the differences in the age structure of the two populations (Texas and California)

```{r}
glmb_s <- glm(cbind(Deaths, Population-Deaths)~ factor(Age_Group)+State, data=txca, family=binomial)

glmb_s%>%
  tbl_regression(exp = TRUE)
```

Which shows that Texas has a standardized mortality rate 17 percent higher than California. 
